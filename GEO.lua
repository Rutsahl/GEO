...
function get_sets()
	...
	IdleIndex = 1
	IdleArray = {"Refresh","Luopan"} -- Default Idle Set Is Movement. Movement removed from cycle 5/23/2020 --
	Armor = 'None'
	StunIndex = 0
	...
	select_default_macro_book() -- Change Default Macro Book At The End --
		 ...
		 -- Binds setup for scenarios -- 
	send_command('bind f9 gs c C6') -- Idle Toggle; Refresh -> Movement -> Luopan -> repeat
	send_command('bind f10 gs c C9') -- MB Toggle; Default 'OFF' as stated in line 15.
	...
	-- PDT Set --
	sets.PDT = set_combine(sets.Idle.Refresh,{
			sub="Genbu's shield",
	...
	sets.Precast.Ice = {main='Daybreak'}
	sets.Precast.Wind = {main='Daybreak'}
	sets.Precast.Earth = {main='Daybreak'}
	sets.Precast.Light = {main='Daybreak'}
	sets.Precast.Dark = {main='Daybreak'}
	...
	sets.WS.Exudation = {}
	sets.WS["Hexa Strike"] = {neck="Fotia gorget"}
	sets.WS.Realmrazer = {}
	sets.WS.Shattersoul = {}
	...
	if spell.action_type == 'Magic' then
		...
		elseif spell.english == 'Monomi: Ichi' then
			...
			if equipSet[spell.type] then
               ...
               if Low_Tier_Spells:contains(spell.english) and LowNuke == 'ON' then
                    equipSet = set_combine(equipSet,sets.LowNuke)
               end
               if spell.skill == 'Elemental Magic' and MB == 'ON' then
